...
class MineSweeper extends World {
  ...
  boolean gameOver;
  boolean gameWin; //true if win, false if lose
  boolean inputHeight;
  boolean inputWidth;
  boolean inputBomb;
  String inputState;

  //constructor allowing user to set gridWidth and gridHeight
  ...
  //Constructor with no parameters, sets board to a default 4x4 grid
  //with two bombs
  MineSweeper() {
    this.gridHeight = 4;
    this.gridWidth = 4;
  ...
  //determines if the game has been won by checking if the number of flags on bombs
  //is equal to the number of bombs and if the # flagged + the # clicked is equal to the 
  ...
  public WorldScene makeScene() {
    ...
    if (this.inputWidth || this.inputHeight || this.inputBomb) {
      return scene;
    }

    //check if the game is over, or if the game is won
...
class Cell {
  ...
  //returns true if this cell is a bomb and clicked and not flagged
  // (the game over state)
  boolean gameOver() {
